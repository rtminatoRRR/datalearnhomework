...
select 100+row_number() over(), ship_mode from (select distinct ship_mode from datalearn.orders ) a;
--checking
...
select 100+row_number() over(), customer_id, customer_name from (select distinct customer_id, customer_name from datalearn.orders ) a;
--checking
...
select 100+row_number() over(), country, city, state, postal_code from (select distinct country, city, state, postal_code from datalearn.orders ) a;
--checking data
...
select 100+row_number() over () as prod_id ,product_id, product_name, category, subcategory, segment from (select distinct product_id, product_name, category, subcategory, segment from datalearn.orders ) a;
--checking
...
--CALENDAR
-- used these article as example for query https://tapoueh.org/blog/2017/06/postgresql-and-the-calendar/
